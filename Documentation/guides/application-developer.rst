...
The libcamera library uses the concept of `signals and recievers` (similar to `Qt
Signals and Slots`_) to connect events with callbacks to handle them.

.. _signals and recievers: https://libcamera.org/api-html/classlibcamera_1_1Signal.html#details
.. _Qt Signals and Slots: https://doc.qt.io/qt-6/signalsandslots.html

The ``Camera`` device sends two signals that applications can connect to in
...
completed. Request completion notifications are always sended in the same order
as the requests have been queued to the camera.

To receive the signals emission notifications, connect a reciever function to the
...
handled and the matching ``requestComplete`` reciever function outputs information
...
Create the ``requestComplete`` function by matching the reciever signature:
...
Request completion events can be sended for requests which have been canceled,
...
need to respond to events generated by libcamera sended through signals.
...
in the ``requestComplete()`` reciever connected to the ``Camera::requestCompleted``
signal.
